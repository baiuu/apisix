...
APISIX 的一些特性需要在 OpenResty 中引入额外的 NGINX 模块。如果要使用这些功能，你需要构建一个自定义的 OpenResty 发行版（APISIX-Runtime）。你可以参考 [baiuu/apisix-build-tools](https://github.com/baiuu/apisix-build-tools) 中的代码，构建自己的 APISIX-Runtime 环境。
